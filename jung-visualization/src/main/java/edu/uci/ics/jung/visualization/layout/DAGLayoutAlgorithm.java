...
public class DAGLayoutAlgorithm<N, P> extends SpringLayoutAlgorithm<N, P> {
  ...
  public DAGLayoutAlgorithm(DomainModel<P> domainModel) {
    super(domainModel);
  }
  ...
  private void initializeLocation(N node, P coord, int width, int height) {
    ...
    domainModel.setLocation(coord, x, y);
  ...
  protected void moveNodes() {
    ...
    synchronized (layoutModel) {
      for (N node : graph.nodes()) {
        ...
        // height.
        double delta = domainModel.getY(xyd) - minY;
        ...
        if (domainModel.getY(xyd) < minY) {
          domainModel.setLocation(xyd, domainModel.getX(xyd), minY);
          // (JY addition: replace height with maxY)
        } else if (domainModel.getY(xyd) > maxY) {
          domainModel.setLocation(xyd, domainModel.getX(xyd), maxY);
        }

        // (JY addition: if there's only one root, anchor it in the
        // middle-top of the screen)
        if (numRoots == 1 && level == 0) {
          domainModel.setLocation(xyd, width / 2, domainModel.getY(xyd));
        }
  ...
  public void setLocation(N picked, P p) {
    setLocation(picked, domainModel.getX(p), domainModel.getY(p));
  }
  ...
  protected void relaxEdges() {
    ...
    for (EndpointPair<N> endpoints : graph.edges()) {
      ...
      double vx = domainModel.getX(p1) - domainModel.getX(p2);
      double vy = domainModel.getY(p1) - domainModel.getY(p2);
