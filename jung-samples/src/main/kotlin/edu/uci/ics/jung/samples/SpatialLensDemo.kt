 ...
 * @author Tom Nelson
 */
class SpatialLensDemo : JPanel() {

    /** the graph */
    private val graph: Network<String, Number>

    private val graphLayoutAlgorithm: LayoutAlgorithm<String>
    ...
    private val magnifyViewSupport: LensSupport
    ...
    private val magnifyLayoutSupport: LensSupport

    private val scaler: ScalingControl
    ...
    init {
        ...
        val visualizationModel = BaseVisualizationModel(graph, graphLayoutAlgorithm, preferredSize)
    ...
    internal class VerticalLabelUI(private val clockwise: Boolean) : BasicLabelUI() {
        ...
        override fun paint(g: Graphics, c: JComponent) {
            ...
            val viewInsets = c.getInsets(paintViewInsets)

            paintViewR.x = viewInsets.left
            paintViewR.y = viewInsets.top

            // Use inverted height & width
            paintViewR.height = c.getWidth() - (viewInsets.left + viewInsets.right)
            paintViewR.width = c.getHeight() - (viewInsets.top + viewInsets.bottom)
    ...
    companion object {
        private val log = LoggerFactory.getLogger(SpatialLensDemo::class.java)

        @JvmStatic
        fun main(args: Array<String>) {
            val f = JFrame()
            f.defaultCloseOperation = WindowConstants.EXIT_ON_CLOSE
            f.contentPane.add(SpatialLensDemo())
            f.pack()
            f.isVisible = true
        }
    }
