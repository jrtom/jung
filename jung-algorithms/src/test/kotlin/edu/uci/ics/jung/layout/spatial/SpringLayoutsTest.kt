 ...
 *
 *  * SpringLayoutAlgorithm - the JUNG legacy version
 *  * SpringBHLayoutAlgorithm - modified to use a BarnesHutQuadTree to reduce the number of
 *    repulsion comparisons with a custom Iterator
 *  * SpringBHVisitorLayoutAlgorithm - modified to use the BarnesHutQuadTree as a visitor during
 *    the repulsion step.
 *
...
class SpringLayoutsTest {
  ...
  fun setup() {
    ...
    layoutModel =
      TestLayoutModel(
        LoadingCacheLayoutModel.builder<String>().setGraph(graph).setSize(500, 500), 30
      )
  ...
  @Test
  fun testSpringLayoutAlgorithm() {
    val layoutAlgorithmOne = SpringLayoutAlgorithm<String>()
  ...
  @Test
  fun testSpringBHLayoutAlgorithm() {
    val layoutAlgorithmTwo = SpringBHIteratorLayoutAlgorithm<String>()
    // using the same random seed each time for repeatable results from each test.
  ...
  @Test
  fun testSpringBHVisitorLayoutAlgorithm() {
    val layoutAlgorithmThree = SpringBHVisitorLayoutAlgorithm<String>()
    // using the same random seed each time for repeatable results from each test.
  ...
  companion object {
    private val log = LoggerFactory.getLogger(SpringLayoutsTest::class.java)
