...
public class TestUnweightedShortestPath extends TestCase {
  ...
  public void testUndirected() {
    UndirectedGraph<String, Integer> ug = new UndirectedSparseMultigraph<String, Integer>();
    for (int i = 0; i < 5; i++) {
    ...
    UnweightedShortestPath<String, Integer> usp = new UnweightedShortestPath<String, Integer>(ug);
  ...
  public void testDirected() {
    DirectedGraph<String, Integer> dg = new DirectedSparseMultigraph<String, Integer>();
    for (int i = 0; i < 5; i++) {
    ...
    UnweightedShortestPath<String, Integer> usp = new UnweightedShortestPath<String, Integer>(dg);
    Assert.assertEquals(usp.getDistance(id.inverse().get(0), id.inverse().get(3)).intValue(), 2);
    Assert.assertEquals(
        (usp.getDistanceMap(id.inverse().get(0)).get(id.inverse().get(3))).intValue(), 2);
    Assert.assertNull(usp.getIncomingEdgeMap(id.inverse().get(0)).get(id.inverse().get(0)));
