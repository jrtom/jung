...
public class TestBarabasiAlbert extends TestCase {
  ...
  /**
   * Due to the way the Barabasi-Albert algorithm works there should be no opportunities for the
   * generation of self-loops within the graph.
   */
        ...
        new Supplier<Graph<Integer, Number>>() {
          public Graph<Integer, Number> get() {
            return new UndirectedSparseGraph<Integer, Number>() {
              ...
              /**
               * This anonymous class works as an UndirectedSparseGraph but will not accept edges
               * that connect a vertex to itself.
               */
              @Override
              public boolean addEdge(
                  Number edge, Pair<? extends Integer> endpoints, EdgeType edgeType) {
                if (endpoints == null)
                ...
                if (v1.equals(v2)) throw new IllegalArgumentException("No self-loops");
                else return super.addEdge(edge, endpoints, edgeType);
              }
            };
          }
        };
  ...
  public void testPreconditions() {
    ...
    try {
      generateAndTestSizeOfBarabasiAlbertGraph(
          graphFactory, vertexFactory, edgeFactory, init_vertices, 0, random_seed, num_tests);
      fail();
    } catch (IllegalArgumentException e) {
    }
    ...
    try {
      generateAndTestSizeOfBarabasiAlbertGraph(
          graphFactory, vertexFactory, edgeFactory, init_vertices, -1, random_seed, num_tests);
      fail();
    } catch (IllegalArgumentException e) {
    }
    ...
    try {
      generateAndTestSizeOfBarabasiAlbertGraph(
          graphFactory, vertexFactory, edgeFactory, 2, 3, random_seed, num_tests);
      fail();
    } catch (IllegalArgumentException e) {
    }
  }
  ...
  /**
   * Every node should have an out-degree AT LEAST equal to the number of edges added per timestep
   * (dependent on if it is directed or undirected).
   */
  ...
  /**
   * Check that not every edge goes to one node; the in-degree of any node should be strictly less
