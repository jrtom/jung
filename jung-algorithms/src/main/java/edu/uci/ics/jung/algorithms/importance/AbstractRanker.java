...
/**
 * Abstract class for algorithms that rank nodes or edges by some "importance" metric. Provides a
 * common set of services such as:
 *
 * <ul>
 *   <li>storing rank scores
 *   <li>getters and setters for rank scores
 *   <li>computing default edge weights
 *   <li>normalizing default or user-provided edge transition weights
 *   <li>normalizing rank scores
 *   <li>automatic cleanup of decorations
 *   <li>creation of Ranking list
 *   <li>print rankings in sorted order by rank
 * </ul>
 *
 * <p>By default, all rank scores are removed from the vertices (or edges) being ranked.
 *
 * @author Scott White
 */
public abstract class AbstractRanker<V, E> extends IterativeProcess {
  ...
  protected LoadingCache<Object, Map<V, Number>> vertexRankScores =
      CacheBuilder.newBuilder()
          .build(
              new CacheLoader<Object, Map<V, Number>>() {
  ...
  protected LoadingCache<Object, Map<E, Number>> edgeRankScores =
      CacheBuilder.newBuilder()
          .build(
              new CacheLoader<Object, Map<E, Number>>() {
  ...
  /** @return <code>true</code> if this ranker ranks nodes, and <code>false</code> otherwise. */
  public boolean isRankingNodes() {
    return mRankNodes;
  }

  /** @return <code>true</code> if this ranker ranks edges, and <code>false</code> otherwise. */
  public boolean isRankingEdges() {
    return mRankEdges;
  }

  /**
   * Instructs the ranker whether or not it should remove the rank scores from the nodes (or edges)
   * once the ranks have been computed.
   *
   * @param removeRankScoresOnFinalize <code>true</code> if the rank scores are to be removed,
  ...
  /**
   * Retrieves the list of ranking instances in descending sorted order by rank score If the
   * algorithm is ranking edges, the instances will be of type <code>EdgeRanking</code>, otherwise
  ...
  /**
   * Given a node, returns the corresponding rank score. This is a default implementation of
   * getRankScore which assumes the decorations are of type MutableDouble. This method only returns
   * legal values if <code>setRemoveRankScoresOnFinalize(false)</code> was called prior to <code>
   * evaluate()</code>.
   *
  ...
  public double getVertexRankScore(V v) {
    ...
    if (rankScore != null) {
      ...
      throw new RuntimeException(
          "setRemoveRankScoresOnFinalize(false) must be called before evaluate().");
    }
  }
  ...
  /**
   * Allows the user to specify whether or not s/he wants the rankings to be normalized. In some
   * cases, this will have no effect since the algorithm doesn't allow normalization as an option
   *
