...
/**
 * Assigns weights to directed edges (the edge of the vertex/edge pair) depending on whether the
 * vertex is the edge's source or its destination. If the vertex v is the edge's source, assigns
 * 1/outdegree(v). Otherwise, assigns 1/indegree(w). Throws <code>IllegalArgumentException</code> if
...
public class UniformInOut<V, E> implements Function<VEPair<V, E>, Double> {
  ...
  public Double apply(VEPair<V, E> ve_pair) {
    ...
    return 1.0 / (graph.isSource(v, e) ? graph.outDegree(v) : graph.inDegree(v));
  }
}
