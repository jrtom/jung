...
public class DirectionTransformer {
  ...
  /**
   * Transforms <code>graph</code> (which may be of any directionality) into an undirected graph.
   * (This may be useful for visualization tasks). Specifically:
   *
   * <ul>
   ...
   *   <li>Each undirected edge (if any) in <code>graph</code> is 'recreated' with a new undirected
   *       edge in the new graph if <code>create_new</code> is true, or copied from <code>graph
   *       </code> otherwise.
   * </ul>
   *
   ...
   */
  public static <V, E> UndirectedGraph<V, E> toUndirected(
      Graph<V, E> graph,
    ...
    for (V v : graph.getVertices()) out.addVertex(v);
    ...
    for (E e : graph.getEdges()) {
      ...
      E to_add;
      if (graph.getEdgeType(e) == EdgeType.DIRECTED || create_new) to_add = edge_factory.get();
      else to_add = e;
  ...
  /**
   * Transforms <code>graph</code> (which may be of any directionality) into a directed graph.
   * Specifically:
   *
   * <ul>
   *   <li>Vertices are copied from <code>graph</code>.
   *   <li>Undirected edges are 'converted' into two new antiparallel directed edges in the new
   *       graph.
   *   <li>Each directed edge (if any) in <code>graph</code> is 'recreated' with a new edge in the
   *       new graph if <code>create_new</code> is true, or copied from <code>graph</code>
   *       otherwise.
   * </ul>
   *
   * @param graph the graph to be transformed
   ...
   * @return the transformed <code>Graph</code>
   */
  public static <V, E> Graph<V, E> toDirected(
      Graph<V, E> graph,
      Supplier<DirectedGraph<V, E>> graph_factory,
      Supplier<E> edge_factory,
      boolean create_new) {
