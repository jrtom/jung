...
public class WeightedNIPaths<V, E> extends AbstractRanker<V, E> {
  ...
  protected void computeWeightedPathsFromSource(V root, int depth) {
    ...
    while (currentDepth <= depth) {
      ...
      for (E currentEdge : edges) {
        incrementRankScore(
            getGraph().getEndpoints(currentEdge).getSecond(), //
            currentWeight);
      }
      ...
      for (E currentSourceEdge :
          edges) { //Iterator sourceEdgeIt = edges.iterator(); sourceEdgeIt.hasNext();) {
        ...
        for (E currentDestEdge : outs) {
          ...
          if (destEdgeDest
              == getGraph()
                  .getEndpoints(currentSourceEdge)
                  .getFirst()) { //currentSourceEdge.getSource()) {
            continue;
          }
  ...
  /**
   * Given a node, returns the corresponding rank score. This implementation of <code>getRankScore
   * </code> assumes the decoration representing the rank score is of type <code>MutableDouble
   * </code>.
   *
