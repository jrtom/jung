...
public class KKLayoutAlgorithm<N, P> extends AbstractIterativeLayoutAlgorithm<N, P>
  ...
  public void step() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      xydata[pm] =
          pointModel.newPoint(
              pointModel.getX(xydata[pm]) + dxy[0], pointModel.getY(xydata[pm]) + dxy[1]);
      ...
      for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
          ...
          if (energy > xenergy) {
            ...
            xydata[i] = pointModel.newPoint(pointModel.getX(xydata[j]), pointModel.getY(xydata[j]));
            xydata[j] = pointModel.newPoint(sx, sy);
            return;
          }
        }
      }
  ...
  public void adjustForGravity() {
    ...
    for (int i = 0; i < xydata.length; i++) {
      ...
      xydata[i] =
          pointModel.newPoint(
              pointModel.getX(xydata[i]) + diffx, pointModel.getY(xydata[i]) + diffy);
