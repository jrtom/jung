...
/**
 * Algorithm variant of <code>PageRankWithPriors</code> that computes the importance of a node based
 * upon taking fixed-length random walks out from the root set and then computing the stationary
 * probability of being at each node. Specifically, it computes the relative probability that the
 * markov chain will spend at any particular node, given that it start in the root set and ends
 * after k steps.
 *
 * <p>A simple example of usage is:
 *
 * <pre>
...
public class KStepMarkov<V, E> extends RelativeAuthorityRanker<V, E> {
  ...
  private static final String CURRENT_RANK =
   ...
   * @param k positive integer parameter which controls the relative tradeoff between a distribution
   *     "biased" towards R and the steady-state distribution which is independent of where the
   *     Markov-process started. Generally values between 4-8 are reasonable
