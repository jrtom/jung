...
/**
 * Simple generator of an m x n lattice where each vertex is incident with each of its neighbors (to
 * the left, right, up, and down). May be toroidal, in which case the vertices on the edges are
 * connected to their counterparts on the opposite edges as well.
 *
 * <p>If the graph Supplier supplied has a default edge type of {@code EdgeType.DIRECTED}, then
 * edges will be created in both directions between adjacent vertices.
 *
...
public class Lattice2DGenerator<V, E> implements GraphGenerator<V, E> {
  ...
  /**
   * Constructs a generator of square lattices of size {@code latticeSize} with the specified
   * parameters.
   *
   ...
   */
  public Lattice2DGenerator(
      Supplier<? extends Graph<V, E>> graph_factory,
      Supplier<V> vertex_factory,
  ...
  /**
   * Creates a generator of {@code row_count} x {@code col_count} lattices with the specified
   * parameters.
   *
   * @param graph_factory used to create the {@code Graph} for the lattice
   * @param vertex_factory used to create the lattice vertices
  ...
  /**
   * Returns the number of edges found in a lattice of this generator's specifications. (This is
   * useful for subclasses that may modify the generated graphs to add more edges.)
   *
