...
public abstract class AbstractIterativeScorer<V, E, T>
  ...
  /**
   * Indicates whether the output and current values are in a 'swapped' state. Intended for internal
   * use only.
   */
  ...
  public AbstractIterativeScorer(
      Hypergraph<V, E> g, Function<? super E, ? extends Number> edge_weights) {
    this.graph = g;
  ...
  /**
   * Specifies whether hyperedges are to be treated as self-loops. If they are, then potential will
   * flow along a hyperedge a vertex to itself, just as it does to all other vertices incident to
   * that hyperedge.
   *
  ...
  /**
   * Returns the effective number of vertices incident to this edge. If the graph is a binary
   * relation or if hyperedges are treated as self-loops, the value returned is {@code
   * graph.getIncidentCount(e)}; otherwise it is {@code graph.getIncidentCount(e) - 1}.
   *
