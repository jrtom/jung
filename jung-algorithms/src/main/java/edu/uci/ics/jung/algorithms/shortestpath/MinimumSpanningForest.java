...
public class MinimumSpanningForest<V, E> {
  ...
  /**
   * Creates a Forest from the supplied Graph and supplied Supplier, which is used to create a new,
   * empty Forest. If non-null, the supplied root will be used as the root of the tree/forest. If
   * the supplied root is null, or not present in the Graph, then an arbitrary Graph vertex will be
   * selected as the root. If the Minimum Spanning Tree does not include all vertices of the Graph,
   * then a leftover vertex is selected as a root, and another tree is created.
   *
   ...
   */
  public MinimumSpanningForest(
      Graph<V, E> graph, Supplier<Forest<V, E>> Supplier, V root, Map<E, Double> weights) {
    this(graph, Supplier.get(), root, weights);
  }

  /**
   * Creates a minimum spanning forest from the supplied graph, populating the supplied Forest,
   * which must be empty. If the supplied root is null, or not present in the Graph, then an
   * arbitrary Graph vertex will be selected as the root. If the Minimum Spanning Tree does not
   * include all vertices of the Graph, then a leftover vertex is selected as a root, and another
   * tree is created
   *
   ...
   */
  public MinimumSpanningForest(
      Graph<V, E> graph, Forest<V, E> forest, V root, Map<E, Double> weights) {
