 ...
 * an mxn (optionally toroidal) lattice. Each node u has four local connections, one to each of its
 * neighbors, and in addition 1+ long range connections to some node v where v is chosen randomly
 * according to probability proportional to d^-alpha where d is the lattice distance between u and v
 * and alpha is the clustering exponent.
 *
...
public class KleinbergSmallWorldGenerator<V, E> extends Lattice2DGenerator<V, E> {
  ...
  /**
   * Sets the {@code Random} instance used by this instance. Useful for unit testing.
   *
