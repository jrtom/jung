...
/**
 * A generalized interface is a mechanism for returning (x,y) coordinates from vertices. In general,
 * most of these methods are used to both control and get information from the layout algorithm.
 *
 * <p>
 *
 * @author danyelf
...
public interface Layout<V, E> extends Function<V, Point2D> {
  ...
  /**
   * Locks or unlocks the specified vertex. Locking the vertex fixes it at its current position, so
   * that it will not be affected by the layout algorithm. Unlocking it allows the layout algorithm
   * to change the vertex's position.
   *
