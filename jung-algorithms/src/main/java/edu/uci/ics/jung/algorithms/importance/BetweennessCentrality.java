...
/**
 * Computes betweenness centrality for each vertex and edge in the graph. The result is that each
 * vertex and edge has a UserData element of type MutableDouble whose key is
 * 'centrality.BetweennessCentrality'. Note: Many social network researchers like to normalize the
 * betweenness values by dividing the values by (n-1)(n-2)/2. The values given here are
 * unnormalized.
 *
