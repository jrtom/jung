 ...
 * graph is a biconnected component if at least 2 vertices must be removed in order to disconnect
 * the graph. (Graphs consisting of one vertex, or of two connected vertices, are also biconnected.)
 * Biconnected components of three or more vertices have the property that every pair of vertices in
 * the component are connected by two or more vertex-disjoint paths.
 *
...
public class BicomponentClusterer<V, E> implements Function<Graph<V>, Set<Set<V>>> {
   ...
   * v</code>, marking each vertex with a value that indicates the order in which it was encountered
