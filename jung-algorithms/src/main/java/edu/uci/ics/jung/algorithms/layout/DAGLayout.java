...
public class DAGLayout<V, E> extends SpringLayout<V, E> {
   ...
   * minimumLevel that is used. We will modify the SpringLayout calculations so that nodes cannot
  ...
  protected void relaxEdges() {
    for (E e : getGraph().getEdges()) {
      ...
      // JY addition.
      int level1 = minLevels.get(v1).intValue();
      int level2 = minLevels.get(v2).intValue();
      ...
      f = f * Math.pow(stretch / 100.0, (getGraph().degree(v1) + getGraph().degree(v2) - 2));
