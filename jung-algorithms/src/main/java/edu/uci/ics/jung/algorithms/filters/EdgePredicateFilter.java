 ...
 * same vertex and edge objects). All vertices from the original graph are copied into the new graph
 * (even if they are not incident to any edges in the new graph).
 *
...
public class EdgePredicateFilter<V, E> implements Filter<V, E> {
  ...
  public Graph<V, E> apply(Graph<V, E> g) {
    ...
    for (V v : g.getVertices()) filtered.addVertex(v);

    for (E e : g.getEdges()) {
      if (edge_pred.apply(e)) filtered.addEdge(e, g.getIncidentVertices(e));
    }
