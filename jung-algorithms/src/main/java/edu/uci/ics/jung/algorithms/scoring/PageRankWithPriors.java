...
public class PageRankWithPriors<V, E> extends AbstractIterativeScorerWithPriors<V, E, Double> {
   ...
   */
  public PageRankWithPriors(
      Hypergraph<V, E> graph, Function<V, Double> vertex_priors, double alpha) {
    ...
    for (E e : graph.getInEdges(v)) {
      ...
      for (V w : graph.getIncidentVertices(e)) {
        if (!w.equals(v) || hyperedges_are_self_loops)
          v_input += (getCurrentValue(w) * getEdgeWeight(w, e).doubleValue() / incident_count);
      }
    }
