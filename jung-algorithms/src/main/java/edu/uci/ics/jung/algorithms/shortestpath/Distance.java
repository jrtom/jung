...
public interface Distance<V> {
  ...
  /**
   * Returns a <code>Map</code> which maps each vertex in the graph (including the <code>source
   * </code> vertex) to its distance (represented as a Number) from <code>source</code>. If any
   * vertex is not reachable from <code>source</code>, no distance is stored for that vertex.
   *
