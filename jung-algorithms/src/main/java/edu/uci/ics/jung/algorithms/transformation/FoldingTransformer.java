 ...
 *
 * <p>A "folded" graph is derived from an input graph by identifying a subset of nodes which
 * will become the nodes of the new graph, copying these nodes into the new graph, and then
 * connecting those nodes that were connected indirectly in the input graph through nodes not in
 * that subset.  This subset is conventionally (but not necessarily) a partition of
 * a k-partite graph.
 *
...
// (b) a utility method that identifies partitions of a k-partite graph public class FoldingTransformer<N, E> {
  /**
   * Converts {@code graph} into a graph <i>T</i> by "folding" {@code graph} "around"
	 * the node set {@code nodes}.  <i>T</i>'s nodes will be {@code nodes}; for any two nodes
	 * {@code a} and {@code b} in {@code nodes}, <i>T</i> will contain the edge
	 * {@code (a, b)} if, and only if, {@code graph} contains a node {@code c} such that
	 * <ul>
	 ...
	 * </ul>
	 *
   * <p>The properties of <i>T</i> (such as directedness) are taken from the properties
	 * of the input graph
	 * (see {@link com.google.common.graph.GraphBuilder#from(com.google.common.graph.Graph)}).
   *
   * @param <N> node type
   ...
   * <p>The properties of <i>T</i> (such as directedness) are taken from the properties
	 ...
	 * (see {@link com.google.common.graph.GraphBuilder#from(com.google.common.graph.Graph)}).
   *
   * <p><i>T</i>'s edge values are the sets of nodes that connected the edge's endpoints in
   * {@code graph}.
   *
   * @param <N> node type
   * @param graph input graph
