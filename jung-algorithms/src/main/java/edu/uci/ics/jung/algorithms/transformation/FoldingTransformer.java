...
public class FoldingTransformer<V, E> {
   ...
   * the old graph; the edges in the new graph are created by the input edge <code>Factory</code>.
   *
   * <p>If there is more than 1 such vertex <code>c</code> for a given pair <code>(a,b)</code>, the
   * type of the output graph will determine whether it will contain parallel edges or not.
   *
   ...
   */
  public static <V, E> Graph<V, E> foldKPartiteGraph(
      KPartiteGraph<V, E> g,
      Predicate<V> p,
      Supplier<Graph<V, E>> graph_factory,
  ...
  /**
   * Converts <code>g</code> into a unipartite graph whose vertices are the vertices of <code>g
   * </code>'s partition <code>p</code>, and whose edges consist of collections of the intermediate
   * vertices from other partitions. For vertices <code>a</code> and <code>b</code> in this
   * partition, the resultant graph will include the edge <code>(a,b)</code> if the original graph
   * contains edges <code>(a,c)</code> and <code>(c,b)</code> for at least one vertex <code>c</code>
   * .
   *
   * <p>The vertices of the new graph are the same as the vertices of the appropriate partition in
   * the old graph; the edges in the new graph are collections of the intermediate vertices <code>c
   * </code>.
   *
   * <p>This function will not create self-loops.
   *
   ...
   * @param p predicate specifying vertex partition
   * @param graph_factory Supplier used to create the output graph
   * @return the result of folding g into unipartite graph whose vertices are those of the <code>p
   *     </code> partition of g
   */
   ...
   *
   * <p>The vertices of the new graph are the same objects as the vertices of <code>h</code>, and
   * <code>a</code> is connected to <code>b</code> in the new graph if the corresponding vertices in
   * <code>h</code> are connected by a hyperedge. Thus, each hyperedge with <i>k</i> vertices in
   * <code>h</code> induces a <i>k</i>-clique in the new graph.
   *
   * <p>The edges of the new graph consist of collections of each hyperedge that connected the
   * corresponding vertex pair in the original graph.
   *
   * @param <V> vertex type
   * @param <E> input edge type
  ...
  public static <V, E> Graph<V, Collection<E>> foldHypergraphEdges(
      Hypergraph<V, E> h, Supplier<Graph<V, Collection<E>>> graph_factory) {
    Graph<V, Collection<E>> target = graph_factory.get();

    for (V v : h.getVertices()) target.addVertex(v);
   ...
   * @return a copy of the input graph where hyperedges are replaced by cliques
   */
  public static <V, E> Graph<V, E> foldHypergraphEdges(
      Hypergraph<V, E> h, Supplier<Graph<V, E>> graph_factory, Supplier<E> edge_factory) {
  ...
  /**
   * Creates a <code>Graph</code> which is a vertex-folded version of <code>h</code>, whose vertices
   * are the input's hyperedges and whose edges are induced by adjacent hyperedges in the input.
   *
   * <p>The vertices of the new graph are the same objects as the hyperedges of <code>h</code>, and
   * <code>a</code> is connected to <code>b</code> in the new graph if the corresponding edges in
   * <code>h</code> have a vertex in common. Thus, each vertex incident to <i>k</i> edges in <code>h
   * </code> induces a <i>k</i>-clique in the new graph.
   *
  ...
  public static <V, E, F> Graph<E, F> foldHypergraphVertices(
      Hypergraph<V, E> h, Supplier<Graph<E, F>> graph_factory, Supplier<F> edge_factory) {
    Graph<E, F> target = graph_factory.get();

    for (E e : h.getEdges()) target.addVertex(e);
   ...
   * </code> induces a <i>k</i>-clique in the new graph.
   *
   * <p>The edges of the new graph consist of collections of each vertex incident to the
   * corresponding hyperedge pair in the original graph.
   *
   * @param h hypergraph to be folded
   * @param graph_factory Supplier used to generate the output graph
   * @return a transformation of the input graph whose vertices correspond to the input's hyperedges
   *     and edges are induced by hyperedges sharing vertices in the input
   */
  public Graph<E, Collection<V>> foldHypergraphVertices(
      Hypergraph<V, E> h, Supplier<Graph<E, Collection<V>>> graph_factory) {
   ...
   * @param incident
   */
  private static <S, T> void populateTarget(
