 ...
 * the algorithm is executed, the input {@code Map} is populated with a {@code Number} for each edge
 * that indicates the flow along that edge.
 *
...
public class EdmondsKarpMaxFlow<V, E> extends IterativeProcess {
  ...
  public EdmondsKarpMaxFlow(
    ...
    try {
      ...
      for (E e : mOriginalGraph.getEdges()) {
        mFlowGraph.addEdge(
            e, mOriginalGraph.getSource(e), mOriginalGraph.getDest(e), EdgeType.DIRECTED);
      }
  ...
  protected void initializeIterations() {
    ...
    for (int eIdx = 0; eIdx < edgeList.size(); eIdx++) {
      ...
      if (capacity == null) {
        throw new IllegalArgumentException(
            "Edge capacities must be provided in Function passed to constructor");
      }
