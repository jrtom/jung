 ...
 * <code>Predicate</code>. The filtered graph is a copy of the original graph (same type, uses the
 * same vertex and edge objects). Only those edges whose entire incident vertex collection passes
 * the predicate are copied into the new graph.
 *
...
public class VertexPredicateFilter<V, E> implements Filter<V, E> {
  ...
  public Graph<V, E> apply(Graph<V, E> g) {
    ...
    for (V v : g.getVertices()) if (vertex_pred.apply(v)) filtered.addVertex(v);

    Collection<V> filtered_vertices = filtered.getVertices();

    for (E e : g.getEdges()) {
      Collection<V> incident = g.getIncidentVertices(e);
      if (filtered_vertices.containsAll(incident)) filtered.addEdge(e, incident);
    }
