...
public class BarabasiAlbertGenerator<V, E> implements EvolvingGraphGenerator<V, E> {
  ...
  private void evolveGraph() {
    ...
    Set<Pair<V>> added_pairs =
        createRandomEdges(preexistingNodes, newVertex, mNumEdgesToAttachPerStep);
  ...
  private Set<Pair<V>> createRandomEdges(
      Collection<V> preexistingNodes, V newVertex, int numEdges) {
  ...
  private void createRandomEdge(
      Collection<V> preexistingNodes,
      V newVertex,
      Set<Pair<V>> added_pairs,
