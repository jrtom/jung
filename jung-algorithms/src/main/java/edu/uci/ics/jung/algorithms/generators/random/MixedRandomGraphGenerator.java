...
/**
 * Generates a mixed-mode random graph (with random edge weights) based on the output of <code>
 * BarabasiAlbertGenerator</code>. Primarily intended for providing a heterogeneous sample graph for
 * visualization testing, etc.
 */
public class MixedRandomGraphGenerator {

  /**
   * Returns a random mixed-mode graph. Starts with a randomly generated Barabasi-Albert
   * (preferential attachment) generator (4 initial vertices, 3 edges added at each step, and
   * num_vertices - 4 evolution steps). Then takes the resultant graph, replaces random undirected
   * edges with directed edges, and assigns random weights to each edge.
   *
  ...
  public static <V, E> Graph<V, E> generateMixedRandomGraph(
    ...
    BarabasiAlbertGenerator<V, E> bag =
        new BarabasiAlbertGenerator<V, E>(
            graphFactory,
            vertexFactory,
            edgeFactory,
            4,
