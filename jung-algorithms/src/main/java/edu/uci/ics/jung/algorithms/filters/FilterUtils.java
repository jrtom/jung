...
public class FilterUtils {
  /**
   * Creates the induced subgraph from <code>graph</code> whose vertex set is equal to <code>
   * vertices</code>. The graph returned has <code>vertices</code> as its vertex set, and includes
   * all edges from <code>graph</code> which are incident only to elements of <code>vertices</code>.
   *
   * @param <V> the vertex type
   * @param <E> the edge type
   * @param <G> the graph type
   * @param vertices the subset of <code>graph</code>'s vertices around which the subgraph is to be
   *     constructed
   * @param graph the graph whose subgraph is to be constructed
   * @return the subgraph induced by <code>vertices</code>
   * @throws IllegalArgumentException if any vertex in <code>vertices</code> is not in <code>graph
   *     </code>
   */
  ...
  public static <V, E, G extends Hypergraph<V, E>> G createInducedSubgraph(
    ...
    try {
      ...
      for (E e : graph.getEdges()) {
        ...
        if (vertices.containsAll(incident)) subgraph.addEdge(e, incident, graph.getEdgeType(e));
      }
  ...
  /**
   * Creates the induced subgraphs of <code>graph</code> associated with each element of <code>
   * vertex_collections</code>. Note that these vertex collections need not be disjoint.
   *
   * @param <V> the vertex type
   * @param <E> the edge type
   * @param <G> the graph type
   * @param vertex_collections the collections of vertex collections to be used to induce the
   *     subgraphs
   * @param graph the graph whose subgraphs are to be created
   * @return the induced subgraphs of <code>graph</code> associated with each element of <code>
   *     vertex_collections</code>
   */
