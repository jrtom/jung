...
public interface MutableCTree<N> extends CTree<N>, MutableGraph<N> {
	 ...
	 * {@inheritDoc}
	 * 
	 * <p>Adds {@code nodeV} as a new successor of {@code nodeU}.
	 * Requirements:
	 * <ul>
	 * <li>{@code nodeU} must be already in the tree, or the tree must be empty
	 * <li>{@code nodeV} must either not be in the tree, or there must be 
	 ...
	 * {@inheritDoc}
	 * 
	 * <p>Removes all nodes from the subtree rooted at {@code node}, so that this graph
	 ...
	 * {@inheritDoc}
	 * 
	 * <p>Removes all nodes from the subtree rooted at {@code nodeV}, so that this graph
	 * continues to be a rooted tree.
	 */
	@Override
