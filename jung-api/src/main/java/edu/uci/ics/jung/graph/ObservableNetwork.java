 ...
 */
public class ObservableNetwork<V, E> implements MutableNetwork<V, E> { // extends MutableNetworkDecorator<V, E> {
	...
	public boolean addEdge(V v1, V v2, E e) {
		...
		if (state) {
			NetworkEvent<V, E> evt = new NetworkEvent.Edge<V, E>(delegate, NetworkEvent.Type.EDGE_ADDED, e);
			fireGraphEvent(evt);
		}
		return state;
	}
	...
	public boolean addNode(V vertex) {
		...
		if (state) {
			NetworkEvent<V, E> evt = new NetworkEvent.Node<V, E>(delegate, NetworkEvent.Type.VERTEX_ADDED, vertex);
			fireGraphEvent(evt);
		}
		return state;
	}
	...
	public boolean removeEdge(E edge) {
		...
		if (state) {
			NetworkEvent<V, E> evt = new NetworkEvent.Edge<V, E>(delegate, NetworkEvent.Type.EDGE_REMOVED, (E) edge);
			fireGraphEvent(evt);
		}
		return state;
	}
	...
	public boolean removeNode(V vertex) {
		...
		if (state) {
			NetworkEvent<V, E> evt = new NetworkEvent.Node<V, E>(delegate, NetworkEvent.Type.VERTEX_REMOVED, (V) vertex);
			fireGraphEvent(evt);
		}
		return state;
	}
